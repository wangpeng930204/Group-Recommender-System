...
def viewed_matrix(ratings_cold_start, all_films):
    ...
    for i in range(len(item_ids)):
        ...
        try:
            ...
            print(
                'item id missing %s' % current_item_id)  ## possibly the movies lacking info such as actors which are discarded
...
def film_strength(user_id, film_id, films, ratings, all_actors, all_directors, all_genres, all_similarities,
                  testing_users_cold_start_for_user, movies_genres, movies_directors, movies_actors):
    ...
    if avg_rat is None:
        item_strength = ((MUG * avgGenreRating) + (MUA * avgActorRating) + (MUD * avgDirectorRating)) / (
                MUG + MUA + MUD)
    else:
        item_strength = ((MUR * avg_rat) + (MUG * avgGenreRating) + (MUA * avgActorRating) + (
                MUD * avgDirectorRating)) / (MUR + MUG + MUA + MUD)
    ...
    for user_id, true_ratings in compressed_test_ratings_dict.items():
        if true_ratings:
            for (film_id, str_rating) in true_ratings:
                strength = film_strength(user_id, film_id, films, ratings, all_actors, all_directors, all_genres,
                                         all_similarities, testing_users_cold_start[user_id], movies_genres,
